...
const createAccount = tool({
  ...
  parameters: z.object({
    ...
    "Priority Image": z.string().optional().describe("Image priorities for the account."),
  }),
  execute: async (fields) => {
    ...
    try {
      ...
      // Fetch allowed values for Priority Image
      const allowedPriorityImages = ["AI Generated", "Google Images", "Stock Images", "Uploaded Media", "Social Media"];

      // Validate Priority Image
      if (fields["Priority Image"] && !allowedPriorityImages.includes(fields["Priority Image"])) {
        return {
          message: `The provided Priority Image option "${fields["Priority Image"]}" is not valid. Please choose one of the following: ${allowedPriorityImages.join(
            ", "
          )}.`,
          logs,
        };
      ...
      if (!fields["Priority Image"]) {
        autoGeneratedFields["Priority Image"] = ""; // Default to blank
      }
      ...
      return {
        ...
        **Priority Image:** ${updateFields["Priority Image"] || "Not provided"}
        **Status:** Draft
