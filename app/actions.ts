...
// Helper function to send requests to Airtable API
async function sendAirtableRequest(endpoint: string, method: string, body?: any) {
...
const createAccount = tool({
  ...
  execute: async ({ name, description }) => {
    ...
    // Send to Airtable
    await sendAirtableRequest("Accounts", "POST", {
      fields: {
        Name: name,
...
const modifyAccount = tool({
  ...
  execute: async ({ accountNumber, fieldToUpdate, newValue }) => {
    ...
    const records = await sendAirtableRequest(
      `Accounts?filterByFormula={AccountNumber}="${accountNumber}"`,
      "GET"
    );

    if (records.records.length > 0) {
      const recordId = records.records[0].id;
      await sendAirtableRequest(`Accounts/${recordId}`, "PATCH", {
        fields: {
